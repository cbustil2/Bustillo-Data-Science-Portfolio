...
st.write("**Heatmap of Missing Vallues**")
...
#There are some missing data with regards to age. 
...
# ================================================================================
# Let user select a numeric column to handle missing data
...
method = st.radio("Choose a method", ["Original DF", "Drop Rows", 
    "Drop Coumns (>50% Missing)", "Impute Mean", "Impute Median", "Impute Zero"])
...
if method == "Original DF":
    ...
    #Or df_clean.dropna(inplace = True)
elif method == "Drop Coumns (>50% Missing)":
    # Drop columns with 50% or more missing values 

    df_clean.drop(columns = df_clean.columns[df_clean.isnull().mean() > 0.5], inplace = True)
